...
float3 AmbientColor = float3(.25, .25, .25);
float3 DiffuseColor = float3(.75, .75, .75);

float3 LightDirection = float3(1, 1, 1);
float3 LightColor = float3(1,1,1);
float3 LightPosition = float3(1000, 100, 1000);
...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	//Apply fog effect - znak
	float dist = 1000/*length(input.ViewDirection)*/;
	float fog = clamp((dist - FogStart) / (FogEnd - FogStart), 0, 1);

	return float4(lerp(diffuseColor*totalLight, FogColor, fog) /*diffuseColor * totalLight*/, 1);
}
